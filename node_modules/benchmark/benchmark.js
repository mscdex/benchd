...
;(function(window, undefined) {
   ...
   */
  function getSource(fn, altSource, alwaysUseAltSource) {
    var result = altSource;
    if (!alwaysUseAltSource) {
      if (isStringable(fn)) {
        result = String(fn);
  ...
  function calcLineOffset(whole, part) {
    var start = whole.indexOf(part);
    if (start <= 0)
      return 0;
    return countLines(whole.slice(0, start));
  }
  ...
  function clock() {
    ...
    clock = function(clone) {
      ...
      if (bench.options.vanilla) {
        source = {
          setup: preprocess('so$=new vm.Script(m$.fnOrig, "__benchd_bench");sb$=vm.createContext();vm.runInContext(m$.setup, sb$, "__benchd_setup");'),
          fn: preprocess('so$.runInContext(sb$);'),
          fnArg: fnArg,
          teardown: preprocess('vm.runInContext(m$.teardown, sb$, "__benchd_teardown");')
        };
        /*source = {
          setup: preprocess('sb$={};vm.runInNewContext(m$.setup, sb$, "__benchd_setup");m$.fn=vm.runInNewContext(m$.fnOrig, sb$, "__benchd_bench");'),
          fn: preprocess('m$.fn(' + fnArg + ');'),
          fnArg: fnArg,
          teardown: preprocess('vm.runInNewContext(m$.teardown, sb$, "__benchd_teardown");')
        };*/
      } else {
        source = {
      ...
      benchCode = interpolate(
        preprocess(deferred
          ...
          : 'var so$,sb$,r$,s$,m$=this,f$=m$.fn,i$=m$.count,n$=t$.ns;#{setup}\n#{begin};' +
            'while(i$--){#{fn}\n}#{end};#{teardown}\nreturn{elapsed:r$,uid:"#{uid}"}'),
        source
      );
      var compiled = bench.compiled = createFunction(preprocess('t$'), benchCode);
      var compiledStr = compiled.toString();
      ...
      try {
        ...
        if (!bench.options.vanilla) {
          bench.lines = {
            baseOffset: calcLineOffset(compiledStr, benchCode),
            setup: countLines(source.setup) + 1,
            // Account for newline immediately following setup
            fn: countLines(source.fn) + 1
          };
        }
        compiled = null;
      ...
      if (decompilable && !compiled && !deferred && !isEmpty) {
        benchCode = interpolate(
          preprocess(
            (clone.error && !stringable
              ? 'var so$,sb$,r$,s$,m$=this,f$=m$.fn,i$=m$.count'
              : 'function f$(){#{fn}\n}var so$,sb$,r$,s$,m$=this,i$=m$.count'
            ) +
        ...
        compiled = createFunction(preprocess('t$'), benchCode);
        compiledStr = compiled.toString();
